...
- **Meridian KMS Portal** (`/research/knowledge-systems/meridian`): Standalone knowledge repository portal — the "product" that the ingestion demo feeds into. Pre-seeded with all 29 Meridian docs on startup via `seedMeridianKMS()` (idempotent). Approvals in the demo pipeline also push to this shared KMS session in real-time. Features: dark portal header with MIG branding + search, stats strip, sidebar category/doctype filters, 3-column responsive doc card grid, grid/list view toggle, right-side detail panel on click (classification metadata, confidence bar, provenance). Auto-refreshes every 10 seconds. "Ingestion Pipeline →" link connects back to demo. Linked from the knowledge-systems index page and from the demo's repository tab header ("Open KMS Portal" button). Route: `GET /api/meridian/kms` (queries `meridian-kms-public` session).
