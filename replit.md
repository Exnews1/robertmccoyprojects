...
- **Signal Flow Animation** (`/research/signal-flow`): A cinematic HTML5 Canvas animation telling the story of the CMGF signal from individual service member to national policy scale in four acts (75 seconds). Act I: single SM with multi-lane back-and-forth advisory, hot glowing ESO signal. Act II: hundreds of SMs at one installation, heavy AI-ESO traffic. Act III: 8 named military bases (Ft Liberty, Ft Cavazos, JBLM, etc.) each with own AI+ESO, trunk pipeline with normalization/aggregation, white PII-stripped beam. Act IV: 180+ installations, 2M+ SMs, hundreds of ESOs feeding the Pentagon. Includes play/pause/reset controls, fullscreen mode for conference presentation, and progress tracking. All client-side, no API calls. Component at `client/src/pages/signal-flow-animation.tsx`.
