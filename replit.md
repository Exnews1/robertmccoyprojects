...
- **Meridian Industrial Group KMS Demo** (`/research/knowledge-systems/demo`): An interactive AI/Human-in-the-loop document processing pipeline demo, branded for "Meridian Industrial Group." Features a three-panel layout: (1) Document Library — 32 pre-loaded industrial documents (MIG-001 through MIG-032) selectable individually or in bulk; (2) Staging Queue — AI classification proposals (doc type, subject, department, standardized name, confidence score) with Approve / Reject / Modify actions per document; (3) Repository + Audit Trail — approved docs organized by department with immutable chronological audit log (INGEST, CLASSIFY, NORMALIZE, APPROVE, REJECT, MODIFY events). Session-scoped via UUID stored in localStorage. Reset button wipes session DB and restores all 32 docs to available state. Rules-based classification engine in `server/meridianClassification.ts`. Backend routes at `/api/meridian/*`. DB tables: `meridian_staging`, `meridian_repository`, `meridian_audit`. Featured card on the knowledge-systems index page. Dark navy/slate branding. Note banner explains rules-based engine vs. production LLM API approach.
