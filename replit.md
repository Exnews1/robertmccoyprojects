...
- **Meridian Industrial Group KMS Demo** (`/research/knowledge-systems/demo`): AI/Human-in-the-loop ingestion pipeline. Selects from 29 real Meridian docs or file uploads (.txt/.pdf/.docx, 10MB). Three panels: Document Library → Staging Queue (classify/approve/reject) → Repository + Audit Trail. Session-scoped. On approval, docs are written BOTH to the user's session repository AND to the shared `meridian-kms-public` session (live-feeds the KMS portal). Backend: `server/meridianClassification.ts` (filename + content-based at 97% confidence). Routes at `/api/meridian/*`. DB: `meridian_staging`, `meridian_repository`, `meridian_audit`.
- **Meridian KMS Portal** (`/research/knowledge-systems/meridian`): Standalone knowledge repository portal — the "product" that the ingestion demo feeds into. Pre-seeded with all 29 Meridian docs on startup via `seedMeridianKMS()` (idempotent). Approvals in the demo pipeline also push to this shared KMS session in real-time. Features: dark portal header with MIG branding + search, stats strip, section nav (Document Library / Accounts Receivable / Accounts Payable), sidebar with year filter + subject/doctype filters, 3-column responsive doc card grid, grid/list view toggle, right-side detail panel on click (classification metadata, confidence bar, provenance). Auto-refreshes every 10 seconds. "Ingestion Pipeline →" link connects back to demo. Routes: `GET /api/meridian/kms` (documents), `GET /api/meridian/kms/financials?type=AR|AP` (financial records).
- **Meridian Financial Records**: 400 synthetic financial records seeded via `seedMeridianFinancials()` on startup (idempotent). Stored in `meridian_financials` table. AR: 200 records (125 paid 2022–2025 with deposit references, 75 open 2025–2026). AP: 200 records (125 paid 2022–2025 with chain-of-evidence payment references via ACH/Check/Wire, 75 pending 2025–2026). KMS portal shows summary cards (collected/outstanding/total), sortable table, year+status filters, and click-to-expand detail rows with full payment chain. Financial records are NOT reset when demo is cleared (they are permanent historical data).
