...
const CHANNELS: Record<string, { id: string; label: string; short: string; color: string; icon: string; desc: string; signals: string[]; tierExamples: string }> = {
...
function DemoBadge({ text, color, filled = false, mono = false }: { text: string; color: string; filled?: boolean; mono?: boolean }) {
  return (
    <span style={{
...
function ModeIndicator({ mode }: { mode: string }) {
  if (mode === "isr") return <DemoBadge text="ISR AGGREGATE" color={C.amber} filled />;
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      <div style={{
        ...
        animation: "cmgf-pulse 2s ease-in-out infinite",
      }} />
      <DemoBadge text={isSandbox ? "EXPLORATION MODE" : "ACTION MODE"} color={isSandbox ? C.sandbox : C.transaction} filled />
      {!isSandbox && <DemoBadge text="SIGNAL GENERATING" color={C.transaction} mono />}
    </div>
  );
}

function ChannelTab({ ch, active, onClick }: { ch: string; active: boolean; onClick: () => void }) {
  const c = CHANNELS[ch];
  return (
...
function ChannelCard({ chId, active, expanded, onClick }: { chId: string; active: boolean; expanded: boolean; onClick: () => void }) {
  const ch = CHANNELS[chId];
  const m = ISR_METRICS[chId];
  return (
    <div onClick={onClick} style={{
      background: active ? `${ch.color}12` : "rgba(17,24,39,0.4)",
...
function ISRBar({ chId, maxVal = 1300 }: { chId: string; maxVal?: number }) {
  const ch = CHANNELS[chId];
  const m = ISR_METRICS[chId];
...
function FirewallDivider({ active }: { active: boolean }) {
  return (
    <div style={{
      ...
      <div style={{ width: 6, height: 6, borderRadius: "50%", background: active ? C.red : C.dim, animation: active ? "cmgf-pulse 1.5s ease infinite" : "none" }} />
  ...
  return (
    <div style={{ background: C.bg, minHeight: "100vh", color: C.text, fontFamily: FONT }}>
      <style>{`
        ...
        @keyframes cmgf-pulse { 0%,100%{opacity:1;filter:brightness(1)}50%{opacity:.6;filter:brightness(0.8)} }
        @keyframes cmgf-slideIn { from{opacity:0;transform:translateY(16px) scale(0.98)}to{opacity:1;transform:translateY(0) scale(1)} }
        .cmgf-layout { display:flex; }
        .cmgf-side { flex:0 0 240px; padding:12px 14px; border-right:1px solid ${C.border}; overflow-y:auto; display:flex; flex-direction:column; gap:8px; }
        .cmgf-side-right { flex:0 0 220px; padding:12px 14px; border-left:1px solid ${C.border}; overflow-y:auto; display:flex; flex-direction:column; gap:10px; }
        @media(max-width:900px){.cmgf-layout{flex-direction:column!important}.cmgf-side,.cmgf-side-right{flex:none!important;width:100%!important;border-right:none!important;border-left:none!important;border-bottom:1px solid ${C.border};max-height:40vh}}
      `}</style>

      {/* HEADER */}
      <div style={{
        padding: "20px 28px 16px", borderBottom: `1px solid ${C.border}`,
        background: "rgba(17,24,39,0.4)", backdropFilter: "blur(12px)",
      ...
      {/* PHASE TIMELINE */}
      {viewMode === "guided" && (
        <div style={{ padding: "8px 24px", borderBottom: `1px solid ${C.border}`, display: "flex", gap: 2, overflowX: "auto" }}>
          {JOURNEY.map((j, i) => {
      ...
      {/* MAIN LAYOUT */}
      <div className="cmgf-layout" style={{ minHeight: "calc(100vh - 110px)" }}>

        {/* LEFT PANEL */}
        <div className="cmgf-side">
        ...
        <div style={{ flex: 1, padding: "16px 24px", overflowY: "auto" }}>
          {viewMode === "guided" ? (
            <div key={phase} style={{ animation: "cmgf-slideIn 0.4s ease" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12, flexWrap: "wrap" }}>
                <ModeIndicator mode={jp.mode} />
                {jp.channel !== "all" && (
                  <DemoBadge text={CHANNELS[jp.channel]?.label || ""} color={CHANNELS[jp.channel]?.color || C.dim} />
                )}
              </div>
              ...
              <div style={{ padding: 18, borderRadius: 12, border: `1px solid ${C.border}`, marginBottom: 16, background: "rgba(17,24,39,0.4)", backdropFilter: "blur(12px)", boxShadow: "0 4px 20px rgba(0,0,0,0.2)" }}>
              ...
              {isISR && (
                <div style={{ marginTop: 20 }}>
                  ...
                  <div style={{ padding: 16, borderRadius: 10, background: C.card, border: `1px solid ${C.amber}25` }}>
            ...
            <div>
              ...
              {(() => {
                ...
                return (
                  <div key={exploreChannel} style={{ animation: "cmgf-slideIn 0.3s ease" }}>
                    ...
                    <div style={{ padding: 16, borderRadius: 12, border: `1px solid ${ch.color}30`, marginBottom: 14, background: "rgba(17,24,39,0.4)", backdropFilter: "blur(12px)" }}>
          ...
          <div style={{ padding: "10px 12px", borderRadius: 8, background: C.card, border: `1px solid ${C.border}` }}>
            ...
            {[
              ...
              <div key={tier.t} style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 3 }}>
                <span style={{ fontSize: 9, fontWeight: 800, color: tier.color, fontFamily: MONO, width: 14, textAlign: "center" }}>{tier.t}</span>
                <span style={{ fontSize: 10, color: C.mid, flex: 1 }}>{tier.label}</span>
                <span style={{ fontSize: 9, color: C.dim }}>{tier.desc}</span>
              </div>
            ))}
          </div>
          <div style={{ padding: "10px 12px", borderRadius: 8, background: C.card, border: `1px solid ${C.amber}20` }}>
            <div style={{ fontSize: 9, fontWeight: 700, color: C.amber, fontFamily: MONO, marginBottom: 6 }}>ISR AGGREGATE</div>
